...
To analyze our data, we decided to implement two different methods of testing: (1) Likelihood Ratio Test with Ordinal Regression and (2) Ordinal Regression Test. 

## Method 1 - Likelihood Ratio Test with Ordinal Logistic Regression

We decided to apply ordinal regression to test whether the main exposure `prog_exp` has a significant impact on our outcome `difficulty`. The orginal regression is used to facilitate the interaction of dependent variables (having multiple ordered levels) with one or more independent variables. In this case, because we are comparing the fit of multiple different models, we decided to adjust  Bonferroni correction
...
## Method 2 - Ordinal Regression Test
